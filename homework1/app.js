...
fs.readFile(path.join(__dirname, 'task2.txt'), 'utf8', (err, data) => {
    ...
    fs.mkdir(path.join(__dirname, 'tasks'), (err) => {
        ...
        fs.wriiteFile(path.join(__dirname, 'taskMain.txt'), data.toString(), (err) => {
            if (err) {
                console.log(err)
                throw err

            }
            fs.unlink(path.join(__dirname, 'taskMain.txt'), (err) => {
                if (err) {
                    console.log(err)
                    throw err
...
fs.mkdir(path.join(__dirname, 'task3', 'main', 'about'), {recursive: true}, (err) => {
    if (err) {
        console.log(err)
        throw err
    }
})
fs.writeFile(path.join(__dirname, 'main', 'about', 'info.txt'), 'info about user', (err) => {
    if (err) {
        console.log(err)
        throw err
    }
...
fs.readdir(path.join(__dirname, 'task3'), (err, data) => {
    ...
    data.map(value => {
        const valueEnd = value.endsWith('txt')
        if (valueEnd === true) {
            ...
            fs.rename(path.join(__dirname, 'task', 'new' + value), (err) => {
                if (err) {
                    console.log(err);
                    throw err
                }
